...
filter_admin_qa_tasks as (
   ...
   select
       ...
       institution_name,
       completed_at_date_time::timestamp as qa_completed_date,
       convert_timezone('EST', completed_at_date_time)::timestamp as qa_completed_date_est,
       completed_by_user_name,
       task_type,
       task_disposition as qa_disposition,
       status as qa_status,
       time_created,
       time_updated,
   ...
   where lower(task_type) = 'admin qa'

)

select *
from filter_admin_qa_tasks
